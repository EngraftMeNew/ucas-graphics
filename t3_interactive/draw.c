...
void init(void)
{
    glClearColor(color_bg.r, color_bg.g, color_bg.b, 0.0f);
}
...
void display(void)
{
    ...
    /* 用支架颜色盖住底座上边被立柱挡住的那一段线 */
    glLineWidth(3.0f);
    setColor(&color_stand);
    glLine(-0.16, -0.47, 0.16, -0.47);

    /*  三角形 */
    setColor(&color_triangle);
    glTri(-0.15, -0.10,
          0.15, -0.10,
          0.0, 0.15,
          GL_TRIANGLES);
...
void glDisk(double x, double y, double radius)
{
    ...
    /* 圆周上的点 */
    for (angle = 0.0; angle <= 2.0 * PI + 1e-9; angle += delta_angle)
    {
        double vx = x + radius * cos(angle);
        double vy = y + radius * sin(angle);
        glVertex2f((GLfloat)vx, (GLfloat)vy);
