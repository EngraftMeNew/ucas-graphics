...
namespace meshark
{
  ...
  protected:
    ...
    friend struct HalfEdgeMesh;
    template <typename T>
    friend struct EdgeData;
    HalfEdge he;
    int index;
  };

  inline Edge nullEdge()
  {
    return mystl::make_observer<EdgeElement>(nullptr);
  }
  ...
  protected:
    ...
    friend struct HalfEdgeMesh;
    template <typename T>
    friend struct FaceData;
    HalfEdge he;
    int index;
  };
  inline Face nullFace()
  {
    return mystl::make_observer<FaceElement>(nullptr);
  }
    ...
    struct OutgoingHalfEdgeRange
    {
      ...
      struct Iterator
      {
        Iterator &operator++()
        {
          ...
          // For an outgoing halfedge h (tail == v), the next outgoing halfedge
          ...
          // When we circle back to start, mark as end.
          if (it == start || !it)
          {
            it = static_cast<HalfEdge>(nullptr);
          }
